...
if __name__ == "__main__":
    ...
    parser.add_argument("--fold", type=int, help="path to configuration file")
    ...
    sets = Kfold_split(cfg["fmt_terms"], cfg["n_split"])
    if args.fold is None:
        runList = range(cfg["n_split"])
    else:
        runList = [args.fold]
    for k in runList:
        tst_fmt_terms = sorted(sets[k])
        tra_fmt_terms = [f for f in cfg["fmt_terms"] if f not in tst_fmt_terms]
        print(f"{tra_fmt_terms}\n{tst_fmt_terms}\n")

        x_train, y_train = create_dataset(tra_fmt_terms, cfg["range_t_train"], cfg)
        x_valid, y_valid = create_dataset(tra_fmt_terms, cfg["range_t_valid"], cfg)
